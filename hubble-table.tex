...
%  \begin{adjustwidth}{-2cm}{}
  ...
  %\begin{tabular}{llll}
    \toprule
    \multicolumn{4}{c}{15 formules pour le rayon de Hubble: pulsation rapide Bang-Crunch}                   \\
    \cmidrule(r){1-4}
   Item     & Formule     & Valeur(Gal) & Remarques \\
    \midrule
    1 & $\lambdabar_e g(6)$ & 13.82  & Fonction topologique $g(k)=exp(2^{k+1/2})/k$ pour k=6 (d=26 valeur critique)\\
    2 & $\lambdabar_e S_4^5$ & 13.80 & $S_4=2\pi^2a^3$ aire de la sphere 4D de rayon $a \approx 137.036$ \\  
    3 & $\lambdabar_e 2^{128}$ & 13.90  & $R/2 \approx 2^{127}$ Nombre de Lucas dernier terme de la hierarchie combinatoire \\ 
    4 & $\lambdabar_e \pi^{155/2}$ & 13.80  & $\pi$ base de calcul comme dans les series de Riemann: $2^{1/155} \approx \pi^{1/256} \approx (2\pi)^{1/(3\times 137)}$ \\
    5 & $\lambdabar_p WZ^{4}$ & 13.80  & relation $a_G \approx W^8$ [3] \\
    6 & $(\lambdabar_{Ryd} n^{4})^2/\lambdabar_p$ & 13.81 & vient de $ct_K/\lambdabar_e \approx aFWZn$ \\ 
    7 & $(2\lambdabar_{e}/3)(\lambdabar_{CMB}/\lambdabar_{H2})^3$ & 13.90  & Conservation holographique \\
    8 & $(\lambda_{CMB}/(j+1))^2/l_P$ & 13.80  & vient de la relation centrale cosmo-biologique [3]: $\sqrt(Rl_P) \approx \lambda_{mam}$ \\
    9 & $(20/3)N_{Edd}Gm_H/c^2$ & 13.79  & Confirme Eddington et l'existence de la masse noire [3] \\
    10 & $(\lambdabar_{e} (2R/R_N)^{210})$ & 13.85  & Confirme le principe holique et l'hologramme du Grandcosmos de rayon $R_N$  \\
    11 & $2(ct_K/F)^2/\lambdabar_{e}$ & 13.81198  & Elimination de c entre les couplages gravitationel et intermediaire [4] \\
    12 & $(\lambdabar_{e} 2^{128})(1-(137^2+\pi^2+e^2)/pH)$ & 13.8119768  & Montre la symetrie de $\pi$, e and 137 \\
    13 & $(\lambdabar_{e} 2^{128}/d_e^2(m_H/m_p)^6$ & 13.8119768  & Raccorde la constante d'Atiyah \\
    14 & $\lambdabar_e g(6)/(1+\sqrt(137^2+\sqrt(136))/jn)$ & 13.8119768 & Confirmation de 137=136+1 \\
    15 & $2\lambdabar_{e} (pn/H^{2})(g(5)/\ln(2-1/ja^2))^2$ & 13.8119767  & Implique la pertinence de $\ln(2) \approx 2\sqrt(3/5)$  \\
    \bottomrule
  \end{tabular}
  \label{tab:table}
\end{table*}
